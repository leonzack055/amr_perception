...
{
public:
    ReflectorDetectorCircle() : Node("reflector_detector_maker"), detection_id_counter_(0)
    {
        // 参数声明
        ...
        this->declare_parameter("diameter_max", 0.08);
...
private:
    ...
    std::map<DetectionHash, std::pair<int, HistoryEntry>> matchCurrentToHistory(const std::vector<Detection>& current_detections) {
        ...
        for (const auto& [id, entry] : detection_history_) {
            Detection dummy_detection{entry.pose, entry.diameter, entry.confidence,entry.translationW,entry.rotationW};
    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        ...
        cartographer_ros_msgs::msg::LandmarkList poses_array;
        std::vector<cartographer_ros_msgs::msg::LandmarkEntry> poses_;
        ...
        for (int label : unique_labels) {
		    ...
		    if (!current_detections.empty()) {
		        ...
		        best_pose.position = best_detection[0].pose.pose.position;
		        best_pose.orientation = best_detection[0].pose.pose.orientation;
		        poseSimple.tracking_from_landmark_transform = best_pose;
		        poseSimple.translation_weight = best_detection[0].translationW;
		        poseSimple.rotation_weight = best_detection[0].rotationW;
		        poses_.push_back(poseSimple);
		...
		poses_array.landmarks = poses_;
		publisher_->publish(poses_array);
    }
};
