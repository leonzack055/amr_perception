 ...
 * 3. 以车量位姿为基准，进行反光柱的匹配获取id分配；当存在全局位姿时，以车辆中心为半径，进行查找可视范围内的反光柱
 * 进行匹配分配
 * @version 0.1
 * @date 2023-08-10
 *
...
: node_(node), search_range_(10.0), match_threshold_(0.1)
{
  ...
  pose_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
    "/tracked_pose", 10, std::bind(&MatchAssigner::pose_callback, this, std::placeholders::_1));
  //     // 订阅反光柱位姿
  landmark_sub_ = node_->create_subscription<visualization_msgs::msg::MarkerArray>(
    "/landmark_poses_list", 10,
    std::bind(&MatchAssigner::landmark_callback, this, std::placeholders::_1));
...
bool MatchAssigner::isLandmarkDetectorOK()
{
  if (pose_msg_ == nullptr) {
    RCLCPP_WARN(node_->get_logger(), "pose_msg_ is nullptr, 还没收到/tracked_pose消息");
    return false;
  }
  return true;
}
  ...
  const std::vector<Detection> & current_detections)
{
  ...
  if (!base_to_laser_transform_available_) {
    try {
      ...
      geometry_msgs::msg::TransformStamped base2laser_tf =
        tf_buffer_
        ->lookupTransform(
        "base_link", laser_frame_, ::rclcpp::Time(0.),
        timeout);
      base_to_laser_transform_ = transforms::ToRigid3d(base2laser_tf);
      base_to_laser_transform_available_ = true;
    } catch (const tf2::TransformException & ex) {
      RCLCPP_ERROR_STREAM(node_->get_logger(), "TF Buffer Transform exception: " << ex.what());
    }
  ...
  for (int i = 0; i < current_detections.size(); i++) {
    ...
    transforms::Rigid3d current_global_landmark = robot_pose * base_to_laser_transform_ *
      laser_to_landmark;
    // 2. 设定全局的landmark的位姿为Identity
    transforms::Rigid3d global_landmark = transforms::Rigid3d(
      current_global_landmark.translation(),
      Eigen::Quaterniond::Identity());
    // TODO: 设定全局的landmark的位姿为Identity
    global_detections[i].pose.pose = transforms::ToGeometryMsgPose(global_landmark);
  }
  ...
  const std::vector<int> & matched_bars)
{
  ...
  for (int i = 0; i < matched_bars.size(); i++) {
    ...
    double fused_confidence =
      (current_detections[i].confidence,
      +reflector_bars_[matched_bars[i]].g_detection_.confidence * 0.8) / 1.8;

    filtered_detections.push_back(
    {
      fused_pose,
      current_detections[i].diameter,
      fused_confidence,
      current_detections[i].translationW,
      current_detections[i].rotationW
    });
  }
...
{
  // 1. 转化为全局坐标系下的位姿
  std::vector<Detection> global_detections = getGlobalDetections(current_detections);
  // 2. 查找匹配的历史检测
  std::vector<int> matched_detections = matchCurrentToHistory(global_detections);
  // 3. 局部融合
  std::vector<Detection> fused_global_detections = updateTemporalFilter(
    global_detections,
    matched_detections);
  // 3. 分配新ID
  for (int idx = 0; idx < matched_detections.size(); idx++) {
    if (matched_detections[idx] == -1) {
      transforms::Rigid3d global_landmark = transforms::ToRigid3d(
        fused_global_detections[idx].pose.pose);
      // 分配新ID，并进行跟踪记录
      ...
      RCLCPP_ERROR_STREAM(node_->get_logger(), "分配新ID: " << new_reflectorbar.id_);
    } else {
      RCLCPP_INFO_STREAM_THROTTLE(
        node_->get_logger(),
        *node_->get_clock(), 1, "区配旧ID: " << matched_detections[idx]);
    }
  }
  ...
  for (int idx = 0; idx < matched_detections.size(); idx++) {
    if (matched_detections[idx] == -1) {
      RCLCPP_ERROR_STREAM(node_->get_logger(), "当前匹配还有没分匹ID的landmark");
      continue;
    }
    transforms::Rigid3d robot_pose = transforms::ToRigid3d(pose_msg_->pose);
    transforms::Rigid3d global_landmark = transforms::ToRigid3d(
    ...
    local_detections[idx].g_detection_.pose.pose = transforms::ToGeometryMsgPose(laser_landmark);
...
{
  Eigen::Quaterniond pose_quat = transforms::ToEigen(pose_msg_->pose.orientation);
...
bool MatchAssigner::time_check(rclcpp::Time time1, rclcpp::Time time2, double threshold)
{
  return (time1 - time2).seconds() < threshold;
}


void MatchAssigner::pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  pose_msg_ = msg;
}
  ...
  const std::vector<Detection> & current_detections)
{
  ...
  for (const auto & reflector_bar : reflector_bars_) {
    auto reflector_bar_pose = transforms::ToRigid3d(reflector_bar.second.g_detection_.pose.pose);
    double distance = (robot_pose.translation() - reflector_bar_pose.translation()).norm();
  ...
  for (int id = 0; id < current_detections.size(); id++) {
    ...
    transforms::Rigid3d current_detect_landmark = transforms::ToRigid3d(
      current_detections[id].pose.pose);
    for (const auto & optimized_bar_id : optimized_bar_ids) {
      ...
      double d =
        (current_detect_landmark.translation() -
        reflector_bar_pose.translation()).norm();
      if (d < opt_distance) {
        opt_distance = d;
...
void MatchAssigner::landmark_callback(const visualization_msgs::msg::MarkerArray::SharedPtr msg)
{
...
// 处理反光柱位姿
    if (marker_msg.ns == "Landmarks" && marker_msg.header.frame_id == "map") {
      ...
      if (reflector_bars_.find(id) == reflector_bars_.end()) {
        RCLCPP_WARN(node_->get_logger(), "landmark %d 没有经过匹配器就进行了发布，检查代码!", id);
      }
      // 更新表中全局landmarks位姿　
