...
//std::ofstream outFile("./test_cpp_circle_r_45_10.txt");
...
public:
    ReflectorDetectorCircle() : Node("reflector_detector_maker"), detection_id_counter_(0)
    {
        ...
        publisher_ = this->create_publisher<cartographer_ros_msgs::msg::LandmarkEntry>(
            "/lanmark", 10);
...
private:
    ...
    rclcpp::Publisher<cartographer_ros_msgs::msg::LandmarkEntry>::SharedPtr publisher_;
    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
			...
			if (circle.valid && circle.r_squared >= 0.85){
            	...
            	if (diameter > diameter_min && diameter < diameter_max){
				    ...
				    if (circle.avg_residual < this->get_parameter("residual_avg_threshold").as_double() &&
						...
						pose.pose.position.z = circle.radius;
        ...
        if (!current_detections.empty()) {
            ...
            double diameter = (best_detection[0].diameter)/2;
            best_pose.pose.position.z = diameter;
            publisher_->publish(best_pose);
            
            //outFile<< best_pose.pose.position.x <<","<<best_pose.pose.position.y <<","<<best_pose.pose.position.z<<std::endl;
            RCLCPP_INFO(this->get_logger(),
                "发布最终结果：圆心(%.3fm, %.3fm), 半径=%.3fm, 置信度=%.2f",
                best_pose.pose.position.x, best_pose.pose.position.y, best_pose.pose.position.z, best_detection[0].confidence);
        } else {
