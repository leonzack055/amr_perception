...
private:
    ...
    // 定义2D向量类型
using Vector2 = std::vector<double>;

// 创建零向量
Vector2 zeroVector2D() {
    return {0.0, 0.0};
}

// 向量加减法
Vector2 vectorSubtract2D(const Vector2& a, const Vector2& b) {
    return {a[0] - b[0], a[1] - b[1]};
}

Vector2 vectorAdd2D(const Vector2& a, const Vector2& b) {
    return {a[0] + b[0], a[1] + b[1]};
}

// 旋转向量（2x2旋转矩阵乘法）
Vector2 rotateVector2D(const Vector2& vec, double angle_rad) {
    double cos_theta = std::cos(angle_rad);
    double sin_theta = std::sin(angle_rad);
    
    return {
        cos_theta * vec[0] - sin_theta * vec[1],
        sin_theta * vec[0] + cos_theta * vec[1]
    };
}

// 计算小车到反光条的变换（考虑反光条旋转）
void calculateVehicleToReflectorTransform(
    const Vector2& T_lr,     // 反光条在雷达坐标系中的位置
    double theta_lr_rad,     // 反光条相对于雷达的旋转角度（30度）
    const Vector2& T_lv,     // 雷达在小车坐标系中的位置
    double theta_lv_rad,     // 雷达相对于小车的旋转角度
    Vector2& T_rv,           // 输出：小车在反光条坐标系中的位置
    double& theta_rv_rad) {  // 输出：小车相对于反光条的旋转角度
    
    // 计算小车相对于反光条的旋转角度
    // theta_rv = theta_lv - theta_lr
    theta_rv_rad = theta_lv_rad - theta_lr_rad;
    
    // 计算平移向量：需要将雷达相对于小车的平移转换到反光条坐标系
    // T_rv = R_lr⁻¹ * (T_lv - T_lr)
    // 其中 R_lr⁻¹ 是反光条到雷达旋转矩阵的逆（即旋转 -theta_lr）
    Vector2 T_diff = vectorSubtract2D(T_lv, T_lr);
    T_rv = rotateVector2D(T_diff, -theta_lr_rad);
}

// 将点从小车坐标系变换到反光条坐标系
Vector2 transformPointToReflectorFrame(
    const Vector2& point_vehicle,
    const Vector2& T_rv,
    double theta_rv_rad) {
    
    Vector2 rotated_point = rotateVector2D(point_vehicle, theta_rv_rad);
    return vectorAdd2D(rotated_point, T_rv);
}
...
double degToRad(double deg) {
    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        ...
        if (!current_detections.empty()) {
            ...
            // 反光条在雷达坐标系中的位置（X方向，Y方向）
            Vector2 T_lr = {ori_coor[0], ori_coor[1]};
            double theta_lr_rad = ori_coor[2];  // 反光条相对于雷达旋转度
            
            // 雷达在小车坐标系中的位置（X方向0.35米，Y方向0米）
            double std_threshold_lidar_x = this->get_parameter("lidar_to_base_tx").as_double();
            double std_threshold_lidar_y = this->get_parameter("lidar_to_base_ty").as_double();
            double std_threshold_lidar_z = this->get_parameter("lidar_to_base_tz").as_double(); 
            Vector2 T_lv = {std_threshold_lidar_x, std_threshold_lidar_y};
            
            // 雷达相对于小车的旋转角度（180度）
            double theta_lv_rad = std_threshold_lidar_z;
            ...
            // 计算变换
            calculateVehicleToReflectorTransform(T_lr, theta_lr_rad, T_lv, theta_lv_rad, T_rv, theta_rv_rad);
            Vector2 vehicle_origin = {0, 0};
            Vector2 reflector_origin = transformPointToReflectorFrame(vehicle_origin, T_rv, theta_rv_rad);
            ...
            best_pose.pose.position.x = reflector_origin[0];
            best_pose.pose.position.y = reflector_origin[1];
            best_pose.pose.orientation.z = std::sin(theta_rv_rad / 2);
			best_pose.pose.orientation.w = std::cos(theta_rv_rad / 2);
