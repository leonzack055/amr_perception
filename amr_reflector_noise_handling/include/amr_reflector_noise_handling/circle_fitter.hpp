...
public:
     ...
     */
    CircleFitResult robustArcFitting(const std::vector<Point>& points, double distance) const {
    ...
    CircleFitResult fitArcWithRANSAC(const std::vector<Point>& points) const {
        ...
        for (int iter = 0; iter < params_.ransac_iterations; ++iter) {
            ...
            if (inlier_count > best_result.inlier_count || 
                (inlier_count == best_result.inlier_count && fit_error < best_result.fit_error)) {
...
private:
     ...
     */
    CircleFitResult fitCircleFrom3Points(const Point& p1, const Point& p2, const Point& p3) const {
        CircleFitResult result;
        result.is_valid = false;
     ...
     * @return CircleFitResult 拟合结果
     */
    CircleFitResult fitCircleFromInliers(const std::vector<Point>& inliers) const {
        CircleFitResult result;
        result.is_valid = false;
        ...
        result.inlier_ratio = static_cast<double>(result.inlier_count) / inliers.size();
