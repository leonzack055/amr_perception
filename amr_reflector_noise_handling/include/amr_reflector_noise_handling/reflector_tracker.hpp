...
/**
 * @brief Tracked reflector information
 */
...
public:
     ...
     */
    std::vector<TrackedReflector> update(
        const std::vector<DetectedReflector>& detections,
        const rclcpp::Time& timestamp) {
        
        std::vector<TrackedReflector> result;
     ...
     */
    const std::vector<TrackedReflector>& getTracks() const {
...
private:
    ...
    std::vector<TrackedReflector> tracked_reflectors_;  // Active tracks
    rclcpp::Node* node_;       // ROS2 node for logging
    
    /**
     * @brief Extract descriptor from detection
     */
    ReflectorDescriptor extractDescriptor(
        const DetectedReflector& detection,
        double timestamp) {
    ...
    void updateTrack(int track_id,
        ...
        for (auto& track : tracked_reflectors_) {
            if (track.id == track_id) {
                ...
                double alpha = 0.3;
                track.position.x = (1.0 - alpha) * track.position.x + alpha * detection.center.x;
                track.position.y = (1.0 - alpha) * track.position.y + alpha * detection.center.y;
    ...
    void removeExpiredTracks(const rclcpp::Time& current_time) {
        ...
        while (it != tracked_reflectors_.end()) {
            ...
            if (time_diff > max_time_gap_) {
                if (node_) {
                    RCLCPP_DEBUG(node_->get_logger(),
                        "Removed expired track %d (time gap: %.2f s)",
                        it->id, time_diff);
                }
    ...
    void removeUnmatchedTracks(const rclcpp::Time& /*current_time*/) {
        ...
        // Tracks that weren't updated will eventually be removed by removeExpiredTracks
    }
     ...
     */
    void fusePointCloud(TrackedReflector& track,
                      const Point& center,
                      double /*diameter*/) {
