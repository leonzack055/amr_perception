...
struct ShapeFeatures {
    ...
    ShapeFeatures() : eigenvalue1(0), eigenvalue2(0), elongation(0),
                     orientation(0), linearity(0), circularity(0), point_count(0) {}
};
...
public:
     ...
     * @brief 设置分类参数
     */
    void setParams(const ShapeClassificationParams& params) {
        params_ = params;
    }
     ...
     */
    ShapeFeatures computeShapeFeatures(
        ...
        features.circularity = 0.4 * fit_score + 0.4 * inlier_score + 0.2 * coverage_score;
     ...
     * @brief 计算角度覆盖率（公共方法）
     */
    double computeAngularCoverage(const std::vector<Point>& points, const Point& center) const {
        if (points.size() < 2) {
            return 0.0;
        }
